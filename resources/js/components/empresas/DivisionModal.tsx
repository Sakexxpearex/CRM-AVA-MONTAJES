...
export default function DivisionModal({ isOpen, onClose, data, setData, submit, errors, processing, empresas }: Props) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-gray-800 w-full max-w-md rounded-2xl p-8 relative shadow-2xl">
                ...
                <h2 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter mb-8 flex items-center gap-3">
                    <LayoutGrid className="text-[#c1f75e]" size={24} /> Nueva División
                </h2>
                ...
                <form onSubmit={submit} className="space-y-5">
                    ...
                    <button 
                        type="submit" 
                        disabled={processing}
                        className="w-full bg-[#c1f75e] text-black py-4 rounded-lg font-black text-xs uppercase tracking-widest shadow-xl shadow-[#c1f75e]/10 hover:brightness-95 active:scale-[0.98] transition-all disabled:opacity-50"
                    >
                        {processing ? 'Creando...' : 'Confirmar División'}
                    </button>
                </form>
            </div>
        </div>
